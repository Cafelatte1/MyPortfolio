...
# # test2
# selected_features = ["close", "kospi", "obv", "trading_amount", "mfi_signal"]
# test3 (거래대금 vs 거래대금20일선 중 n+3 일의 평균/표편 값이 더 높아 선택)
feature_test_seed = "feature_test3"
selected_features = ["date", "close", "kospi", "obv", "trading_amount", "mfi_signal"]
# test4
feature_test_seed = "feature_test4"
selected_features = ["date", "close", "fast_d", "obv", "fore_mv20", "inst_mv5", "kospi", "trading_amount_mv20", "mfi_signal"]
...
for stock_name, stock_code in stock_list.items():
    ...
    # <feature scaling>
    # log transform
    # ["date", "close", "fast_d", "obv", "fore_mv20", "inst_mv20", "kospi", "trading_amount_mv20", "mfi_signal"]
    ...
    # # ===== hist plot on numerical feature =====
    # for i in ["close", "kospi", "trading_amount"]:
    #     if i in train_x.columns:
    #         plt.figure(figsize=(12, 6))
    ...
    tmp_lm = sm.OLS(target_list[timeunit_gap_forviz][:-timeunit_gap_forviz].to_frame(),
                    sm.add_constant(train_x.drop("date", axis=1), prepend=False)[:-timeunit_gap_forviz])
    ols_obj = tmp_lm.fit()
...
stock_dic["삼성전자"]["df"].isna().sum()
stock_dic["삼성전자"]["df"].head()
...
def createNetwork(nCols, mlpName, hiddenLayers=128, dropoutRate=1/2**2, seqLength=5, eta=3e-3):
    # nCols = 10
    # hiddenLayers = 128
    # dropoutRate = 0.2
    # seqLength = 5
...
for time_ngap in range(1,target_timegap+1):
    ...
    for stock_name, stock_data in stock_dic.items():
        ...
        # train_x_lstm = full_x[:-seqLength]
        # train_y_lstm = full_y[seqLength - 1:]
        ...
        # prediction on test data
        lgb.LGBMRegressor(boosting_type='goss', objective="regression", subsample=0.8,
                          n_estimators=model.best_iteration_,
                          num_leaves=grid.best_params_["num_leaves"],
                          learning_rate=grid.best_params_["learning_rate"],
                          n_jobs=multiprocessing.cpu_count(), random_state=321)
        ...
        model = doMLP(train_x_lstm, train_y_lstm, val_x_lstm, val_y_lstm, mlpName="MLP_LSTM_V1",
                      hiddenLayers=128, epochs=100, batch_size=4, seqLength=seqLength, model_export=True)
        print(model["performance"])
        stock_data["perf_list"]["MLP_LSTM_V1"][time_ngap] = model["performance"]
        tmp_perf = series(model["performance"])

        model = doMLP(full_x_lstm, full_y_lstm, test_x_lstm, test_y_lstm, mlpName="MLP_LSTM_V1",
                      hiddenLayers=128, epochs=100, batch_size=4, seqLength=seqLength, preTrained=model["model"])
